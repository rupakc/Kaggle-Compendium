...
def label_encode_frame(dataframe):
    ...
    for column in columns:
        if type(dataframe[column][1]) is str:
...
target_value = 'shot_made_flag'
del df['game_date']
class_values = list(df['shot_made_flag'].values)
del df[target_value]
df = label_encode_frame(df)
X_train,X_test,y_train,y_test = train_test_split(df.values,class_values,test_size=0.2,random_state=42)
