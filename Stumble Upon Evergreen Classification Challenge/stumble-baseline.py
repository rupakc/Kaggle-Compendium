...
# TODO - Take into account the boilerplate which contains the webpage content
...
filename = 'train.tsv'
stumble_frame = pd.read_csv(filename,sep="\t",na_values='?')
del stumble_frame['url']
del stumble_frame['urlid']
del stumble_frame['boilerplate']
class_labels = list(stumble_frame['label'].values)
del stumble_frame['label']
label_encoder = LabelEncoder()
stumble_frame['alchemy_category'] = label_encoder.fit_transform(stumble_frame['alchemy_category'].values)
stumble_frame.fillna(value=0,inplace=True)

X_train,X_test,y_train,y_test = train_test_split(stumble_frame.values,class_labels,test_size=0.2,random_state=42)
