...
train_users_filename = 'train_users.csv'
sessions_filename = 'sessions.csv'

user_frame = pd.read_csv(train_users_filename)
session_frame = pd.read_csv(sessions_filename)
df_list = [user_frame,session_frame]
merged_frame = reduce(lambda left,right:pd.merge(left,right,left_on='id',right_on='user_id',how='inner'),df_list)
merged_frame.dropna(inplace=True)
del merged_frame['id']
del merged_frame['user_id']
del merged_frame['timestamp_first_active']
merged_frame['created_month'],merged_frame['created_day'],merged_frame['created_year'] = spilt_date(list(merged_frame['date_account_created'].values))
merged_frame['first_month'],merged_frame['first_day'],merged_frame['first_year'] = spilt_date(list(merged_frame['date_first_booking'].values))
del merged_frame['date_account_created']
del merged_frame['date_first_booking']
class_labels = list(merged_frame['country_destination'].values)
