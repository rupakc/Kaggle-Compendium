...
filename_people = 'people.csv'
filename_activity = 'act_train.csv'
people_frame = pd.read_csv(filename_people)
activity_frame = pd.read_csv(filename_activity)
final_frame = pd.merge(people_frame,activity_frame,left_on='people_id',right_on='people_id')
final_frame = label_encode_frame(final_frame)
final_frame.drop(['people_id','activity_id'],axis=1,inplace=True)

for column in final_frame.columns:
    if final_frame[column].isnull().sum() > 10000:
        del final_frame[column]
print len(final_frame)
class_labels = list(final_frame['outcome'].values)
del final_frame['outcome']
X_train,X_test,y_train,y_test = train_test_split(final_frame.values,class_labels,test_size=0.2,random_state=42)
