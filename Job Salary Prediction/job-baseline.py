...
title_features = hashfeatures.FeatureHash(max_feature_num=100).get_feature_set(list(train_frame['Title'].values))
description_features = hashfeatures.FeatureHash(max_feature_num=900).get_feature_set(list(train_frame['FullDescription'].values))
target_values = train_frame['SalaryNormalized'].values
train_frame.drop(['Title','FullDescription','SalaryNormalized'],axis=1,inplace=True)
train_frame = label_encode_frame(train_frame)
final_feature_set = np.hstack((title_features,description_features,train_frame.values))
