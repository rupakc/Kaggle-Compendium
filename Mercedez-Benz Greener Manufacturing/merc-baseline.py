...
filename = 'train.csv'
merc_frame = pd.read_csv(filename)
regression_values = list(merc_frame['y'].values)
del merc_frame['ID']
del merc_frame['y']
encoded_frame = label_encode_frame(merc_frame)
X_train,X_test,y_train,y_test = train_test_split(encoded_frame.values,regression_values,test_size=0.1,random_state=42)
