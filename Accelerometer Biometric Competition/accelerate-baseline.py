...
train_frame['T'] = map(lambda x: float(x)/200000.0,train_frame['T'].values)
device_labels = train_frame['Device'].values
del train_frame['Device']
X_train,X_test,y_train,y_test = train_test_split(train_frame.values,device_labels,test_size=0.2,random_state=42)
