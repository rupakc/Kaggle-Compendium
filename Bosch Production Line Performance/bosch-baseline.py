...
def remove_nan_columns(dataframe,threshold=0.95):
    columns_to_remove = list([])
    min_count_of_nan = int(threshold*len(dataframe))
    for column in dataframe.columns:
        if dataframe[column].isnull().sum() > min_count_of_nan:
...
numeric_frame = pd.read_csv(filename,nrows=100000)
class_labels = list(numeric_frame['Response'].values)
del numeric_frame['Response']
del numeric_frame['Id']
reduced_dataframe = remove_nan_columns(numeric_frame)
imputed_frame = Imputer().fit_transform(reduced_dataframe.values)
X_train,X_test,y_train,y_test = train_test_split(imputed_frame,class_labels,test_size=0.2,random_state=42)
