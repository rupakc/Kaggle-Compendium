...
filename = 'train.csv'
expedia_frame = pd.read_csv(filename)
del expedia_frame['srch_id']
del expedia_frame['date_time']
booking = list(expedia_frame['booking_bool'].values)
del expedia_frame['booking_bool']
impute = Imputer()
imputed_frame = impute.fit_transform(expedia_frame.values)
X_train,X_test,y_train,y_test = train_test_split(imputed_frame,booking,test_size=0.2,random_state=42)
