...
filename = 'train.csv'
property_frame = pd.read_csv(filename)
del property_frame['Id']
predicted_hazard = list(property_frame['Hazard'].values)
del property_frame['Hazard']
encoded_property_frame = label_encode_frame(property_frame)
X_train,X_test,y_train,y_test = train_test_split(encoded_property_frame.values,predicted_hazard,test_size=0.1,random_state=42)
