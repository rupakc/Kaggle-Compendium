...
train_filename = 'train_2016_v2.csv'
property_detail_filename = 'properties_2016.csv'
impute = Imputer()
train_frame = pd.read_csv(train_filename)
property_frame = pd.read_csv(property_detail_filename)
columns_to_drop = [22,32,34,49,55]
property_frame.drop(property_frame.columns[columns_to_drop], axis=1,inplace=True)
combined_frame = pd.merge(train_frame,property_frame,left_on='parcelid',right_on='parcelid')
predicted_values = list(combined_frame['logerror'].values)
del combined_frame['parcelid']
del combined_frame['logerror']
month_list,day_list,year_list = spilt_date(list(combined_frame['transactiondate'].values))
combined_frame['month'] = month_list
combined_frame['day'] = day_list
combined_frame['year'] = year_list
del combined_frame['transactiondate']
label_encoded_frame = label_encode_frame(combined_frame)
imputed_values = impute.fit_transform(combined_frame.values)
X_train,X_test,y_train,y_test = train_test_split(imputed_values,predicted_values,test_size=0.2,random_state=42)
