...
filename = 'train.csv'
amazon_frame = pd.read_csv(filename)
target_class_labels = list(amazon_frame['ACTION'].values)
del amazon_frame['ACTION']
X_train,X_test,y_train,y_test = train_test_split(amazon_frame.values,target_class_labels,test_size=0.2,random_state=42)
X_train,y_train = TomekLinks(random_state=42).fit_sample(X_train,y_train)
