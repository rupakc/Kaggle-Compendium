...
filename = 'train.csv'
bnp_frame = pd.read_csv(filename)
del bnp_frame['ID']
class_labels = list(map(lambda x:int(x),list(bnp_frame['target'].values)))
del bnp_frame['target']
columns = bnp_frame.columns
columns_to_keep = list([])
for column in columns:
    if (type(bnp_frame[column][0]) is np.float64) or (type(bnp_frame[column][0]) is np.int64):
        columns_to_keep.append(column)
filtered_bnp_frame = bnp_frame[columns_to_keep]
imputer = Imputer()
imputed_data = imputer.fit_transform(filtered_bnp_frame.values)
X_train,X_test,y_train,y_test = train_test_split(imputed_data,class_labels,test_size=0.1,random_state=42)
